...
public class AnimalGridGameManager : MonoBehaviour
{
    ...
    private IEnumerator CorrectAnswerSequence(AudioClip animalSound)
    {
        ...
        // 5️⃣ Check and show Interstitial Ad (The core change)
        if (Interstitial.instance != null && Interstitial.instance.IsAdAvailable())
        {
            Debug.Log("Ad available. Showing interstitial and deferring next round...");
            // Show ad, and only call GenerateRound() after the ad closes (via the callback)
            Interstitial.instance.ShowInterstitialAd(GenerateRound);
        }
        else
        {
            Debug.Log("Ad not available or Interstitial manager missing. Starting next round immediately.");
            // If ad is not ready, proceed directly to the next round
            GenerateRound();
        }
    }
    ...
    private IEnumerator FlipCard(RectTransform card, float duration)
    {
        ...
        // Find Back and Animal images
        // Note: We are using the Flip logic to handle the visibility swap
        Image backImage = card.transform.Find("BackImage")?.GetComponent<Image>();
        Image animalImage = card.transform.Find("AnimalImage")?.GetComponent<Image>();
        ...
        // 2️⃣ Swap visibility: Back hides, Animal shows (assuming back was initially visible)
